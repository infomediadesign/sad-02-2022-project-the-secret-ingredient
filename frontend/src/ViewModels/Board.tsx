...
export const dragReducer = produce((state: any, action: any) => {
    switch (action.type) {
        ...
        case 'DELETEISSUE': {
            ...
            deleteCardFromIssueList(action.deleteMe, issueListsMIds[action.myIndex], state[issueListsNames[action.myIndex]].length);
...
export async function bordMainSetup(boardNum: number){
    ...
    if(boardResponse.board.length == 0){
        boardResponse = await postGeneric("http://localhost:1234/Board", {"name": "testBoard", "image" : {"color" : "green", "thumb" : "one.jpg", "full" : "true"}, "uId" : userID }, 'POST' )
    ...
    for(var i = 0; i < issueListsNames.length; i++){
        ...
        for(var j = 0; j < listsCards.cards.length; j++){
            let listsActiv = await getGeneric('http://localhost:1234/card/' + listsCards.cards[j]._id + '/activitys', 'GET');
...
export async function addToIssueListNames(newValue: string){
    ...
    waitingForDb = true;
    const response = await postGeneric("http://localhost:1234/list", {"name" : newValue, "bId" : currentBoardId, "order" : issueListsNames.length-1}, 'POST');
...
export async function addIssue(name: string, index: number, content: string, order: number){
    ...
    const response = await postGeneric("http://localhost:1234/card", {"name" : name, "lId" : issueListsMIds[index], "bId" : currentBoardId, "order" : order}, 'POST');
    currentCardId = response.card._id;
    console.log(response);
    console.log(content);
    console.log(currentCardId);
    console.log(currentBoardId);
    const responseNew = await postGeneric("http://localhost:1234/Activity", {"text" : content, "cId" : currentCardId, "bId" : currentBoardId}, 'POST'); 
...
export async function editIssue(listIndex: string, newText: string){
    ...
    const responseNew = await postGeneric("http://localhost:1234/Activity", {"text" : newText, "cId" : listIndex, "bId" : currentBoardId}, 'POST'); 
}
...
export async function moveIssueExternalList(oldPos : number, newPos : number, newList: string, oldList: string, newListIndex: number){
...
async function moveIssues(issueListId : string, moveAt : number, moveDirect : number, me : number, adding : boolean){
    ...
    response.cards.map(async(item : any) => {
        ...
        if(item.order*moveDirect >= moveAt*moveDirect && me*moveDirect > item.order*moveDirect){
            await postGeneric('http://localhost:1234/Card/'+ item._id, {"name": item.name, "order": (item.order + moveDirect)}, 'PUT')
        }
    });
}